...
MiniProxy.prototype.start = function() {
    var server = http.createServer((req, res) => {
      // if (req.method == 'POST') {
        ...
        req.on('data', function(data) {
            body += data;
            console.log(data.toString('utf8') ,'=============1111111111111111');
        });
        req.on('end', function() {
    ...
    server.on('upgrade', function(req, socket, head) {
        socket.write('HTTP/1.1 101 Web Socket Protocol Handshake\r\n' +
            ...
            'Connection: Upgrade\r\n' +
            '\r\n');
        console.log('upgradeupgradeupgradeupgradeupgrad===')
...
var myProxy = new MiniProxy({
  ...
  "onBeforeResponse": function(v) {
    console.log(v,'1=1=1==1=')
  }
});

myProxy.start();
console.log("proxy start at 9999");

function requestCustom(req, res) {
      console.log('*************************************')
        ...
        try {
            ...
            //u can change request param here
            requestRemote(requestOptions, req, res);
    
        } catch (e) {
            console.log("requestHandlerError: " + e.message);
        }
        
        function requestRemote(requestOptions, req, res) {
            var remoteRequest = https.request(requestOptions, function(remoteResponse) {
                remoteResponse.headers['proxy-agent'] = 'Easy Proxy 1.0';
