...
module.exports = class CertAndKeyContainer {
    ...
    getCertPromise (hostname, port) {
        for (let i = 0; i < this.queue.length; i++) {
            ...
            //     }
            // }
            if(mappingHostNames.length > 0) {
                var DNSName = mappingHostNames[0];
                if (tlsUtils.isMappingHostName(DNSName, hostname)) {
                    this.reRankCert(i);
                    return _certPromiseObj.promise;
                }
        ...
        let promise = new Promise((resolve, reject) => {
            ...
            var preReq = https.request({
                ...
                try {
                    ...
                    if (realCert) {
                        try {
                            certObj = await tlsUtils.createFakeCertificateByCA(this.caKey, this.caCert, realCert);
                        } catch (error) {
                            certObj = await tlsUtils.createFakeCertificateByDomain(this.caKey, this.caCert, hostname);
                        }
                    } else {
                        certObj = await tlsUtils.createFakeCertificateByDomain(this.caKey, this.caCert, hostname);
                    }
            ...
            preReq.setTimeout(~~this.getCertSocketTimeout,async () => {
                if (!certObj) {
                    certObj = await tlsUtils.createFakeCertificateByDomain(this.caKey, this.caCert, hostname);
                    _resolve(certObj);
                }
            });
            preReq.on('error',async (e) => {
                if (!certObj) {
                    certObj = await tlsUtils.createFakeCertificateByDomain(this.caKey, this.caCert, hostname);
                    _resolve(certObj);
                }
