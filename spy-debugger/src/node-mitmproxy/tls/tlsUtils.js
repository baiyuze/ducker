...
utils.createFakeCertificateByDomain = async function (caKey, caCert, domain) {
    var a = Date.now();
    ...
    console.log('TIME:' + ((Date.now() - a) / 1000) + '秒');
    return {
        key: keys.privateKey,
        cert: cert
    };
...
// utils.getMappingHostNamesFormCert = function (cert) {
//     var mappingHostNames = {};
//     var key = cert.subject.getField('CN') ? cert.subject.getField('CN').value : ''
//     mappingHostNames[key] = key;
//     var altNames = cert.getExtension('subjectAltName') ? cert.getExtension('subjectAltName').altNames : [];
//     var altNamesObj = {}
//     altNames.forEach(key => {
//         altNamesObj[key] = key
//     })
//     Object.assign(mappingHostNames, altNamesObj)
//     // mappingHostNames.push(cert.subject.getField('CN') ? cert.subject.getField('CN').value : '');
//     // mappingHostNames = mappingHostNames.concat(_.map(altNames, 'value'));
//     return mappingHostNames;
// }
